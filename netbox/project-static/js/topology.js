...
api_call("/static/js/topology_config.json", "GET", undefined, function(config) {
    ...
    // Get device role colors
    api_call("/api/dcim/device-roles/?limit=0", "GET", undefined, function(response) {
       $.each(response.results, function(index, device_role) {
    ...
    // load connections
    api_call("/api/dcim/cables/?limit=0", "GET", undefined, function(response){
       $.each(response.results, function(index, connection) {

           if (connection.termination_a_type == 'dcim.interface' ||
               connection.termination_a_type == 'dcim.frontport' ||
               connection.termination_a_type == 'dcim.rearport'  ||
               connection.termination_b_type == 'dcim.interface' ||
               connection.termination_b_type == 'dcim.frontport' ||
               connection.termination_b_type == 'dcim.rearport' ) {
               if (connection.color != "") {
               ...
               edges.add({
                   ...
                   title: 'Connection between<br>'
                       +connection.termination_a.device.name+' ['+connection.termination_a.name+']<br>'
                       +connection.termination_b.device.name+' ['+connection.termination_b.name+']',
               });
           }
       });
    });
